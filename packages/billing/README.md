...
Pending or action-required Refunds keep deletion fenced and retryable until a signed `refund.updated` event reports success. Signed `refund.failed`/terminal updates are matched to the durably stored Refund id; after confirming the PaymentIntent has no partial succeeded refund, processing claims a persisted next attempt and retries with an attempt-specific idempotency key. Unknown Refunds are ignored. Partial-refund or otherwise unsafe states remain operator-visible and fail closed.

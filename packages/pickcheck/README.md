...
```bash
npm install -g @pickforge/pickcheck
pickcheck-install
```
...
```bash
pi install npm:@pickforge/pickcheck
```
...
claude plugin install pickcheck@pickforge
```

Alternatively, `pickcheck-install --harness claude` merges the equivalent hooks into `~/.claude/settings.json` without replacing existing hooks.

### Codex

```bash
pickcheck-install --harness codex
cp -r node_modules/@pickforge/pickcheck/codex-skill/pickcheck ~/.codex/skills/
```
...
The installer runs `omp plugin install npm:@pickforge/pickcheck`. OMP uses the same after-edit and agent-end extension as Pi.
...
The installer runs `opencode plugin @pickforge/pickcheck --global`, which updates OpenCode's global config. The plugin appends edit findings to tool output and continues an idle session when changed functions fail.
...
The wrapper resolves the executable in this order: `PICKCHECK_BIN`, the verified binary under `vendor/`, then `pickcheck` on `PATH`. stdin, stdout, stderr, argv, and exit status are inherited unchanged.

...
const TRUNCATED = "... pickcheck output truncated";
...
async function check(pi: ExtensionAPI, cwd: string, args: string[]): Promise<GateResult> {
  try {
    const result = await pi.exec("pickcheck", ["check", ...args], { cwd });
...
export default function pickcheckExtension(pi: ExtensionAPI): void {
  let blocks = 0;
  let warned = false;
  ...
  const warnOnce = (ctx: ExtensionContext, error: string): void => {
    if (warned) return;
    warned = true;
    notify(ctx, `pickcheck skipped: ${error.slice(0, 240)}`);
  };
  pi.on("tool_result", async (event, ctx) => {
    try {
      ...
      notify(ctx, "pickcheck found violations in the edited file");
  ...
  pi.on("agent_end", async (_event, ctx) => {
    try {
      ...
      if (!result.feedback) return;
      notify(ctx, "pickcheck found violations in changed functions");
      if (blocks >= 3) return;
      blocks += 1;

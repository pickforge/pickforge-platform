---
name: pickcheck
description: Measure and reduce function complexity with the pickcheck binary. Use for every coding and code-review task, when a pickcheck hook reports FAIL lines, or when the user asks to refactor or simplify code.
---

# PickCheck

For every coding or code-review task, run `pickcheck check --changed`
...
```bash
pickcheck check <file>        # one file
pickcheck check --changed     # compact list of changed files that need work
pickcheck check --changed --verbose <file>
```
...
- Never raise a limit in `.pickcheck.json` to get green. Legacy code you
  did not touch is not your problem; the gate only checks changed functions.

## Workflow

1. Run `pickcheck check --changed` from inside the repository.

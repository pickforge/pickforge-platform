...
export const hookFragments: Record<"claude" | "codex" | "grok" | "cursor", Json> = {
  claude: {
    hooks: {
      PostToolUse: [{ matcher: "Edit|Write|MultiEdit", hooks: [{ type: "command", command: "pickcheck hook claude" }] }],
      Stop: [{ hooks: [{ type: "command", command: "pickcheck hook claude" }] }],
    },
  },
  codex: {
    hooks: {
      PostToolUse: [{ matcher: "Edit|Write|MultiEdit", hooks: [{ type: "command", command: "pickcheck hook codex" }] }],
      Stop: [{ hooks: [{ type: "command", command: "pickcheck hook codex" }] }],
    },
  },
  grok: {
    hooks: {
      PostToolUse: [{ matcher: "edit|write|apply_patch|Edit|Write|MultiEdit", hooks: [{ type: "command", command: "pickcheck hook grok" }] }],
      Stop: [{ hooks: [{ type: "command", command: "pickcheck hook grok" }] }],
    },
  },
  cursor: {
    version: 1,
    hooks: {
      afterFileEdit: [{ command: "pickcheck hook cursor" }],
      stop: [{ command: "pickcheck hook cursor", loop_limit: 3 }],
    },
  },
};
...
function containsGateHook(value: unknown): boolean {
  if (typeof value === "string") return value.includes("pickcheck hook ");
...
export function spawnPi(command: string, platform = process.platform, spawnProcess = spawn): ReturnType<typeof spawn> {
  const args = ["install", "npm:@pickforge/pickcheck"];
...
export function spawnOmp(command: string, platform = process.platform, spawnProcess = spawn): ReturnType<typeof spawn> {
  const args = ["plugin", "install", "npm:@pickforge/pickcheck"];
  return platform === "win32"
    ? spawnProcess("cmd.exe", ["/d", "/s", "/c", command, ...args], { stdio: "inherit" })
    : spawnProcess(command, args, { stdio: "inherit" });
}
...
export function spawnOpenCode(command: string, platform = process.platform, spawnProcess = spawn): ReturnType<typeof spawn> {
  const args = ["plugin", "@pickforge/pickcheck", "--global"];
  return platform === "win32"
    ? spawnProcess("cmd.exe", ["/d", "/s", "/c", command, ...args], { stdio: "inherit" })
    : spawnProcess(command, args, { stdio: "inherit" });
}
...
async function runPi(): Promise<void> {
  ...
  if (!(await executable(command))) {
    console.log("pi install npm:@pickforge/pickcheck");
    return;
  }
...
async function runOmp(): Promise<void> {
  ...
  if (!(await executable(command))) {
    console.log("omp plugin install npm:@pickforge/pickcheck");
    return;
  }
  await new Promise<void>((resolve, reject) => {
...
async function runOpenCode(): Promise<void> {
  ...
  if (!(await executable(command))) {
    console.log("opencode plugin @pickforge/pickcheck --global");
    return;
  }
  await new Promise<void>((resolve, reject) => {
...
function printHarness(harness: Harness): void {
  if (harness === "pi") console.log("pi install npm:@pickforge/pickcheck");
  else if (harness === "omp") console.log("omp plugin install npm:@pickforge/pickcheck");
  else if (harness === "opencode") console.log("opencode plugin @pickforge/pickcheck --global");
...
async function installHarnesses(harnesses: Harness[], home: string): Promise<void> {
  ...
  if (harnesses.includes("grok")) {
    if (await hasGrokCompatibleHook(home, harnesses)) {
      console.log("grok: using the installed Claude Code or Cursor pickcheck hooks");
    } else {
      await mergeFile(join(home, ".grok", "hooks", "pickcheck.json"), hookFragments.grok);
    }
  }

...
const wrapper = fileURLToPath(new URL("../bin/pickcheck", import.meta.url));
...
async function runAdapter(request: GateRequest): Promise<string | undefined> {
  return new Promise((resolve) => {
    ...
    child.on("error", (error) => {
      console.warn(`pickcheck: ${error.message}`);
      resolve(undefined);
    });
    child.on("close", (code) => {
      if (code !== 0) {
        console.warn(`pickcheck: hook failed${stderr.trim() ? `: ${stderr.trim()}` : ""}`);
        resolve(undefined);
        return;
      }
      try {
        ...
        console.warn("pickcheck: hook returned invalid JSON");
        resolve(undefined);
      }
    });

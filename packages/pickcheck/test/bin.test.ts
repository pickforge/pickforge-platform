...
describe("binary forwarding", () => {
  it("prefers PICKCHECK_BIN and forwards argv and exit code", async () => {
    const root = await mkdtemp(join(tmpdir(), "complexity-bin-"));
    roots.push(root);
    const binary = join(root, "gate");
    await writeFile(binary, "#!/bin/sh\nprintf '%s' \"$*\"\nexit 7\n");
    await chmod(binary, 0o755);
    const result = await execute(process.execPath, [resolve("packages/pickcheck/bin/pickcheck"), "check", "a.ts"], { ...process.env, PICKCHECK_BIN: binary });

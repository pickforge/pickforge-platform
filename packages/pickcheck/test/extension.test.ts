...
import pickcheckExtension from "../extensions/pickcheck.ts";
...
function host(stdout = violation, code = 1, stderr = "") {
  ...
  pickcheckExtension(pi as never);
...
describe("Pi extension", () => {
  it("injects feedback after an edited file violates", async () => {
    ...
    expect(exec).toHaveBeenCalledWith("pickcheck", ["check", "--summary", "src/a.ts"], { cwd: "/repo" });

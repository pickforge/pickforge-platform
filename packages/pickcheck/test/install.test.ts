...
describe("hook installation", () => {
  ...
  it.each(["[]", "42", '{"hooks":"invalid"}', '{"hooks":{"Stop":"x"}}'])("rejects invalid settings without changing %s", async (contents) => {
    ...
    const result = spawnSync(process.execPath, [resolve("packages/pickcheck/src/install.ts"), "--harness", "claude", "--home", home], { encoding: "utf8" });
  ...
  it("runs pi.cmd through cmd.exe on Windows", async () => {
    ...
    expect(spawn).toHaveBeenCalledWith("cmd.exe", ["/d", "/s", "/c", "pi.cmd", "install", "npm:@pickforge/pickcheck"], { stdio: "inherit" });
  });

  it("runs omp.cmd through cmd.exe on Windows", () => {
    const spawn = vi.fn();
    spawnOmp("omp.cmd", "win32", spawn as never);
    expect(spawn).toHaveBeenCalledWith("cmd.exe", ["/d", "/s", "/c", "omp.cmd", "plugin", "install", "npm:@pickforge/pickcheck"], { stdio: "inherit" });
  });

  it("runs opencode.cmd through cmd.exe on Windows", () => {
    const spawn = vi.fn();
    spawnOpenCode("opencode.cmd", "win32", spawn as never);
    expect(spawn).toHaveBeenCalledWith("cmd.exe", ["/d", "/s", "/c", "opencode.cmd", "plugin", "@pickforge/pickcheck", "--global"], { stdio: "inherit" });
  });
  ...
  it("writes native Grok and Cursor configuration", async () => {
    ...
    const grok = JSON.parse(await readFile(join(home, ".grok", "hooks", "pickcheck.json"), "utf8"));
    ...
    expect(cursor.hooks.stop[0]).toMatchObject({ command: "pickcheck hook cursor", loop_limit: 3 });
  });
  ...
  it("reuses compatible hooks instead of running Grok twice", async () => {
    ...
    await expect(readFile(join(home, ".grok", "hooks", "pickcheck.json"))).rejects.toMatchObject({ code: "ENOENT" });
    log.mockRestore();
  });
  ...
  it("prints fragments without writing", async () => {
    ...
    expect(log.mock.calls.flat().join("\n")).toContain("pickcheck hook codex");

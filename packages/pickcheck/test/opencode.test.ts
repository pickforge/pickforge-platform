...
describe("OpenCode plugin", () => {
  it("runs the package wrapper directly outside Windows", () => {
    expect(adapterProcess("linux")).toMatchObject({ command: expect.stringContaining("/bin/pickcheck"), args: [] });
    expect(adapterProcess("win32")).toMatchObject({ command: "node", args: [expect.stringContaining("/bin/pickcheck")] });
  });
  ...
  it("adds complexity feedback to edit tool output", async () => {
    ...
    expect(output.output).toContain("pickcheck feedback");
  });

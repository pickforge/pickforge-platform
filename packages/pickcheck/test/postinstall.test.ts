...
describe("binary downloader", () => {
  it("vendors the binary from the cargo-dist archive directory", async () => {
    ...
    roots.push(root);
    const stem = "pickcheck-x86_64-unknown-linux-gnu";
    await mkdir(join(root, stem));
    await writeFile(join(root, stem, "pickcheck"), "#!/bin/sh\necho fake\n");
    await chmod(join(root, stem, "pickcheck"), 0o755);
    ...
    expect(destination).toBe(join(vendorDir, "pickcheck"));
  ...
  it("uses the configurable cargo-dist asset name with fake fetch", async () => {
    ...
    const checksum = createHash("sha256").update(archive).digest("hex");
    const fetchImpl = vi.fn(async (url: string) => new Response(url.endsWith("sha256.sum")
      ? `${checksum}  pickcheck-x86_64-unknown-linux-gnu.tar.xz\n`
      : archive));
    ...
    expect(fetchImpl.mock.calls[0]![0]).toContain("/v9.8.7/pickcheck-x86_64-unknown-linux-gnu.tar.xz");
  });

  it("rejects a checksum mismatch before extraction", async () => {
    const fetchImpl = vi.fn(async (url: string) => new Response(url.endsWith("sha256.sum")
      ? `${"0".repeat(64)}  pickcheck-x86_64-unknown-linux-gnu.tar.xz\n`
      : "archive"));
  ...
  it("skips download when the binary override is set", async () => {
    const previous = process.env.PICKCHECK_BIN;
    process.env.PICKCHECK_BIN = "/custom/gate";
    const warn = vi.spyOn(console, "warn").mockImplementation(() => {});
    await postinstall();
    expect(warn).toHaveBeenCalledWith(expect.stringContaining("skipping binary download"));
    warn.mockRestore();
    if (previous === undefined) delete process.env.PICKCHECK_BIN; else process.env.PICKCHECK_BIN = previous;
  });
});
